...
export const NucleusActivityContent = ({
  ...
  const offerTokenOption = currentTx?.offerToken
    ? findTokenByAddress(currentTx?.offerToken)
    : null;
  ...
  const steps: Step[] = useMemo(() => {
    ...
    if (currentTx?.status === "fulfilled") {
      ...
      if (swapStatus === StepStatus.COMPLETED) {
        transactionSteps.push({
          title: `Withdraw Fulfillment`,
    ...
    currentTx?.createdTransactionHash,
    currentTx?.endingTransactionHash,
    currentTx?.status,
  ]);
      ...
      <div className="activity-container">
        {isLoading ? (
          <Loading
            ...
            style={{ margin: "auto", transform: "scale(4)" }}
          />
        ) : (
          evmWallet &&
          transactions &&
                ...
                >
                  <div className="transaction-top flex justify-between">
                    <div className="flex tx-age" style={{ gap: "7px" }}>
                      ...
                      <span className="gray-in">
                        {timeAgo(Number(tx.createdTimestamp))}
                      </span>
                    </div>
                    <div
                      className={`flex flex-row items-center status-div ${tx.status}`}
                    >
                      {tx.status ? (
                        <>
                          <TransactionIcon
                            iconType={
                              isExpired
                                ? StepStatus.EXPIRED
                                : eclipseNucleusStatusMap[tx.status]
                            }
                          />
                          <span>
                            {isExpired
                  ...
                  <div className="transaction-bottom flex justify-between">
                    <div
                      className="flex flex-row items-center eth-to-ecl"
                      style={{ gap: "14px" }}
                    >
                      <span className="white-in preserve-casing">
                        {offerTokenOption?.label}
                      </span>
                      <Arrow />
                      <span className="white-in preserve-casing">
                        {wantTokenOption?.label}
                      </span>
                    </div>
