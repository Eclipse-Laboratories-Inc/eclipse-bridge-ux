...
export const WithdrawDetails: React.FC<TransactionDetailsProps> = ({
  ...
  useEffect(() => {
    ...
    setTxHash(
      withdrawal?.transaction ? withdrawal?.transaction.signature : "0xhash"
    );
  ...
  useEffect(() => {
    if (!tx) return;
    if (tx[1] === "Processing")
      setWaitingPeriodStatus(WaitingPeriodState.Waiting);
  ...
  const submitClaim = async () => {
    ...
    // Route every claim to the configured contract address, which should be V3 contract now.
  ...
  return (
    <div
      className={`transaction-details-modal flex flex-col items-center ml-[0px] ${isSidebar ? "sm:ml-[110px]" : "sm:ml-[34px]"}`}
    >
      ...
      <div className="status-panel">
        <div className="panel-elem flex flex-row items-center justify-between">
          <div className="left-side flex flex-row items-center">
            <div
              className={
                initiateStatus !== InitiateTxStates.NotReady || txHash
                  ? "white-text"
                  : "gray-text"
              }
              style={{ fontSize: "16px" }}
            ...
            {txHash && txHash !== "0xhash" && (
              <div className="gray-text">
                <a
                  href={composeEclipsescanUrl(
                    selectedOption,
                    composeEtherscanCompatibleTxPath(txHash)
                  )}
                  target="_blank"
                >
                  View Txn
          ...
          {(initiateStatus !== InitiateTxStates.NotReady || txHash) && (
            ...
            >
              <TransactionIcon
                ...
                isGreen={true}
              />
              <span>{txHash ? "Done" : initiateStatus}</span>
            </div>
          )}
        </div>
          ...
          {(initiateStatus === InitiateTxStates.Done || txHash) && (
            ...
            >
              ...
              <span>
                ...
                waitingPeriodStatus === WaitingPeriodState.Closed ? (
                  "Done"
                ) : (
                  <div className="flex flex-row gap-2 items-center justify-center">
                    ...
                    <p className="green-text">
                      ~{" "}
                      {tx
                        ? timeLeft(parseInt(tx[0].start_time, 16) * 1000)
                        : waitingPeriod}
                    </p>
                  </div>
                )}
            ...
            <div className="gray-text">
              {false && (
                <a
                  href={composeEclipsescanUrl(
                    selectedOption,
                    composeEtherscanCompatibleTxPath(eclipseTx)
                  )}
                  target="_blank"
                >
                  View Txn
      ...
      "
        ...
        >
          ...
          <span className="w-[396px]">
            I understand that it will take {waitingPeriod} until my funds are
