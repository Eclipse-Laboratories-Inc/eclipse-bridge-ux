...
export const NetworkBox: React.FC<NetworkBoxProps> = ({
  ...
  function adjustInputWidth() {
    if (inputRef.current) {
      ...
      refs.style.width =
        (len ? len : 5) +
        (refs.value.toString().includes(".") ? 0 : 0.5) +
        "ch";
    }
  }
      ...
      <div className={`network-info flex items-center ${css}`}>
        <div className="network-info-left-section flex items-center justify-center">
          <img
            src={imageSrc}
            alt=""
            style={{ objectFit: "cover", height: "44px", width: "44px" }}
          />
        ...
        {wallet && (
          <div className="network-info-right-section">
              ...
              <div className="input-wrapper">
                {!evmWallet || (evmWallet && balanceEther >= 0) ? (
                  <>
                    ...
                    />
                    {amountEther && (
                      <span className="font-medium text-[34px] ml-[-4px]">
                        ETH
                      </span>
                    )}
                  </>
                  ...
                  {ethPrice ? (
                    amountEther && amountEther != "." ? (
                      <span className="font-medium">
                        $
                        {(parseFloat(amountEther.toString()) * ethPrice)
                          .toFixed(2)
                          .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" "}
                      </span>
                    ) : (
