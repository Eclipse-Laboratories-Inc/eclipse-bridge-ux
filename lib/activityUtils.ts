...
export async function generateTxObjectForDetails(
  walletClient: any,
  txHash: string,
) {
...
// fix
export async function getEclipseTransaction(
...
// fix
export async function checkDepositWithPDA(
  address: PublicKey | null,
  eclipseRpc: string,
) {
  if (!address) {
...
export async function getLastDeposits(address: string, chain: string) {
  ...
  const response = await fetch(
    `/api/get-transactions?address=${address}&chain=${chain}`,
  );
